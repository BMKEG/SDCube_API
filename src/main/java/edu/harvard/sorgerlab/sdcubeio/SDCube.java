...
public class SDCube {
	...
	/**
	 * Add an ExpDesign_Description/DataModule combo as a single-SAMPLE to the
	 * SDCube
	 * 
	 * @author Bjorn Millard
	 * @param SDCube_Sample
	...
	public void load(String sdcPath) throws H5IO_Exception {
		...
		ArrayList<ExpDesign_Sample> eps = ExpDesign_IO.parseSamples(sdcPath
				+ "/ExpDesign.xml");
			...
			for (int i = 0; i < len; i++) {
				...
				for (int j = 0; j < eps.size(); j++) {
					if (eps.get(j).getId().trim().equals(data.getId().trim())) {
						...
						TheSamples.add(new SDCube_Sample(data, eps.get(j), data
								.getId()));
						break;
					}
				}
	...
	public void write(String sdcubePath) throws H5IO_Exception {
		...
		if (!h5File.exists()) {
			try {
				...
				System.err.println("***ERROR creating H5 file***");
				e.printStackTrace();
			}
		}
		if (!xmlFile.exists()) {
		...
		// Checking if samples already exist, and if so, rename new samples so
		// dont conflict
		...
		if (numSamples > 0) {
			...
			for (int i = 0; i < numS; i++) {
				...
				dm.replacePath(
						"./Children/"
								+ path.substring(indStart, indEnd + 1).trim(),
						"./Children/" + numSamples);
		...
		for(SDCube_Sample s : this.TheSamples ) {
			model.addSample(s.getExpDesign());
		}
	...
	static public ArrayList<SDCube_Sample> getSamples(String sdcPath) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (data != null && expD != null && expD.size() != 0
				...
				System.err.println("**ERROR: loading sample with id: "
						+ ids.get(i));
		}
		return samples;
	}
	 ...
	 *            [] tagNames
	 */
	static public ArrayList<SDCube_Sample> getSamplesWithDescriptorNames_AND(
			String sdcPath, String[] names) {
		ArrayList<SDCube_Sample> samples = new ArrayList<SDCube_Sample>();
		String xmlPath = sdcPath + "/ExpDesign.xml";
